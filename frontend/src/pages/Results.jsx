...
function Results() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#10221a] flex items-center justify-center">
      <div className="flex flex-col items-center gap-3">
        <div className="w-10 h-10 border-3 border-[#0fbd74]/30 border-t-[#0fbd74] rounded-full animate-spin" />
  ...
  if (error) return (
    <div className="min-h-screen bg-[#10221a] flex items-center justify-center px-4">
      <div className="glass-panel rounded-xl p-8 text-center max-w-md">
        ...
        <button onClick={() => navigate('/history')} className="text-[#0fbd74] text-sm font-medium">Return to History</button>
      </div>
    </div>
  )
        ...
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <div className="flex items-center gap-2 text-sm text-[#8faeb0] mb-1">
              ...
              <button onClick={() => navigate('/history')} className="hover:text-[#0fbd74] transition-colors">History</button>
              <span className="material-icons-round text-xs">chevron_right</span>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => navigate('/analyze')} className="bg-[#0fbd74] hover:bg-[#0ca665] text-white px-4 py-2 rounded-lg text-sm font-medium shadow-lg shadow-[#0fbd74]/20 transition-all flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <motion.div
              ...
              className={`glass-panel rounded-xl p-6 border-l-4 ${isHealthy ? 'border-l-[#0fbd74]' : 'border-l-red-500'}`}
            >
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                <div className="flex items-center gap-5">
                  <div className={`w-16 h-16 rounded-2xl flex items-center justify-center ${isHealthy ? 'bg-[#0fbd74]/20' : 'bg-red-500/20'}`}>
                    <span className={`material-icons-round text-3xl ${isHealthy ? 'text-[#0fbd74]' : 'text-red-500'}`}>
                  ...
                  <div>
                    ...
                    <span className={`inline-flex items-center gap-1.5 mt-3 px-3 py-1.5 rounded-full text-sm font-medium ${
                      ...
                      <span className={`w-2 h-2 rounded-full ${isHealthy ? 'bg-[#0fbd74]' : 'bg-red-500 animate-pulse'}`} />
                ...
                <div className="text-center sm:text-right">
                  <div className="relative w-24 h-24 mx-auto sm:mx-0">
                    <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                      <circle cx="50" cy="50" r="42" fill="none" stroke="#1c3b2f" strokeWidth="6" />
                      <circle cx="50" cy="50" r="42" fill="none" stroke={isHealthy ? '#0fbd74' : '#ef4444'} strokeWidth="6" strokeLinecap="round"
              ...
              >
                <div className="flex items-center justify-between mb-5">
                  <h3 className="text-lg font-bold text-white flex items-center gap-2">
                    <span className="material-icons-round text-[#0fbd74]">satellite_alt</span>
                    Zone Health Map
                  </h3>
                  <div className="flex gap-4">
                    <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-[#0fbd74]" /><span className="text-xs text-[#8faeb0]">Healthy</span></div>
                    ...
                    return (
                      <motion.div
                        ...
                        className={`p-5 rounded-xl border transition-all cursor-pointer hover:scale-[1.02] ${
                          zHealthy ? 'bg-[#0fbd74]/5 border-[#0fbd74]/20 hover:border-[#0fbd74]/40' : 'bg-red-500/5 border-red-500/20 hover:border-red-500/40'
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <span className={`material-icons-round ${zHealthy ? 'text-[#0fbd74]' : 'text-red-500'}`}>
                              {zHealthy ? 'check_circle' : 'error'}
                            </span>
                            <div>
                              <p className="text-sm font-semibold text-white">{zone.zone}</p>
                              <p className={`text-xs font-medium ${zHealthy ? 'text-[#0fbd74]' : 'text-red-400'}`}>
            ...
            >
              <div className="flex items-center gap-3 mb-5">
                <div className="p-2 bg-[#0fbd74]/20 rounded-lg">
                  <span className="material-icons-round text-[#0fbd74]">auto_awesome</span>
                </div>
                <div>
              ...
              <div className="space-y-3">
                {aiSteps.map((step, i) => (
                  <motion.div
                    ...
                    className="flex items-start gap-4 p-4 rounded-lg bg-[#0c1b14] border border-white/5 hover:border-[#0fbd74]/20 transition-colors"
                  >
                    <span className="flex-shrink-0 w-7 h-7 rounded-full bg-[#0fbd74]/10 flex items-center justify-center text-xs font-bold text-[#0fbd74]">{i + 1}</span>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            >
              ...
              <div className="space-y-3">
                <button
                  ...
                  disabled={exporting}
                  className="w-full bg-[#0fbd74] hover:bg-[#0ca665] disabled:opacity-50 text-white py-3 rounded-lg font-medium shadow-lg shadow-[#0fbd74]/20 transition-all flex items-center justify-center gap-2 text-sm"
                >
                  <span className="material-icons-round text-lg">picture_as_pdf</span>
                  {exporting ? 'Generating PDF...' : 'Export Full Report'}
                </button>
                <button className="w-full py-3 px-4 rounded-lg text-sm font-medium bg-[#0c1b14] text-gray-300 border border-white/10 hover:border-[#0fbd74]/30 transition-all flex items-center justify-center gap-2">
            ...
            >
              ...
              <div className="space-y-4">
                {[
                  { label: 'Status', value: isHealthy ? 'Healthy' : 'Stressed', color: isHealthy ? 'text-[#0fbd74]' : 'text-red-500' },
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: 'Season Avg', value: '91.2%', icon: 'analytics', color: 'text-[#0fbd74]', bg: 'bg-[#0fbd74]/20' },
                  ...
                  <div key={i} className="p-3 rounded-lg bg-[#0c1b14] border border-white/5 text-center">
